...
<div class="max-w-2xl mx-auto mt-10">
    ...
    <form method="POST" action="{{ route('car-brands.update', $carBrand) }}" class="space-y-4">
        @csrf
        @method('PUT')
        <input name="name" value="{{ $carBrand->name }}" type="text" placeholder="Nom de la marque" class="w-full border p-2" required>
        <input name="country" value="{{ $carBrand->country }}" type="text" placeholder="Pays" class="w-full border p-2" required>
